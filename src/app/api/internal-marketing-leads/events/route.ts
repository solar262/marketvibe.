...
import { getLeadHuntEvents, logLeadHuntEvent, type LeadHuntEventPayload } from "@/lib/internal-marketing-leads";
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ events: await getLeadHuntEvents(limit) }, { headers: INTERNAL_CORS_HEADERS });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Event log read failed" }, { status: 400, headers: INTERNAL_CORS_HEADERS });
  }
}
