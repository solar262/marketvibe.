...
const DEFAULT_PAIN = "need clients, looking for leads, cold outreach not working, prospecting help, SEO clients, agency clients";
      ...
      <section className="relative overflow-hidden border-b border-white/10 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto max-w-5xl">
          <div className="text-center">
            ...
            <h1 className="mt-5 text-4xl font-semibold tracking-tight text-white sm:text-5xl">Manual Facebook opportunity radar.</h1>
            <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-slate-300">
              Paste a public Facebook post, score the buyer intent, copy a helpful reply, open Facebook, and post manually.
            </p>
          </div>
      ...
      <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4 text-center">
            ...
            <p className="mt-2 text-2xl font-semibold text-white">Manual</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Done</p>
            <p className="mt-2 text-2xl font-semibold text-emerald-300">{doneCount}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Skipped</p>
            <p className="mt-2 text-2xl font-semibold text-amber-200">{skippedCount}</p>
          </div>
        </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {(["quick", "deeper"] as ReplyMode[]).map((mode) => (
                  <button
                    ...
                    disabled={mode === "deeper" && !result.deeperReply}
                    className={`rounded-full border px-4 py-2 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-45 ${
                      replyMode === mode ? "border-cyan-200 bg-cyan-200 text-slate-950" : "border-white/15 bg-white/10 text-white hover:bg-white/15"
                    }`}
                  >
